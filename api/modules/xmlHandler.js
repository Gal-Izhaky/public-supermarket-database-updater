...
import geocodeStores from "./geocodingHandler.js";

const parser = new xml2js.Parser();

// get a brand in english and translate it to hebrew
const translateBrand = (brand) => {
    return brand
        ...
        .replaceAll("יוניברס", "שופרסל דיל"); // same thing, consistency is key.
}
...
const handleCerberus = async (parsedXML, brand) => {
    ...
    for (const store of stores) {
        ...
        const isValid =
            address !== "" && // no address
            address.toLowerCase() !== "unknown" && // no address
            address !== "קיבוץ" && // no address
            city.toLowerCase() !== "unknown"  && // no address
            !address.toLowerCase().includes("www.") && // online store
...
const handleShufersal = async (parsedXML) => {
    ...
    for (const store of stores) {
        const type = translateBrand(store.SUBCHAINNAME[0]);

        // check for validity, if valid, format and insert into the array
        const isValid = 
        !exclude.includes(type) &&
        !store.ADDRESS[0].toLowerCase().includes("www."); // online store
        !store.CITY[0].toLowerCase().includes("www."); // online store
...
// function that might be used later, it combines the names of items with different codes 
const combineSameNames = (prices) => {
    // eg. מלפפון in 2 brands have different item codes
    let names = {};
